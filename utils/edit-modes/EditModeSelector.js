...
const { EditMode, DeleteArrow, CreateArrow, SetArrow, MarkupTypes } =
  Autodesk.Extensions.Markup.Core;
 ...
 * @constructor
 */
export const EditModeSelector = (editor) => {
...
proto.onMouseDown = function () {
  ...
  if (editor.snapper) {
    ...
    if (
      geomType === MeasureCommon.SnapType.SNAP_VERTEX ||
  ...
  var head = (this.head = editor.positionFromClientToMarkups(
    this.initialX,
    this.initialY
  ));
  var tail = {
...
proto.onMouseMove = function (event) {
  ...
  if (editor.snapper && !editor.snapper.isSnapped() && this.lineSnapped) {
    ...
    var perpendicular = MeasureCommon.nearestPointInPointToLine(
      p,
      pStart,
      pEnd
    );
  ...
  this.size = editor.sizeFromClientToMarkups(
    final.x - initialX,
    final.y - initialY
  );
